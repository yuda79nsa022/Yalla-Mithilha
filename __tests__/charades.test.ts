...
const titles: CharadesTitle[] = Array.from({ length: 10 }, (_, i) => ({ id: `t${i}`, text: `Title ${i}` }));

describe('draftCharades', () => {
  it('starts locked, with no titles and zero scores', () => {
    const state = draftCharades('deck-1', 'Falcons', 'Hawks');
  ...
  it('generates a unique id by default, but accepts an explicit one', () => {
    const a = draftCharades('deck-1', 'A', 'B');
    const b = draftCharades('deck-1', 'A', 'B');
    expect(a.id).not.toBe(b.id);

    const explicit = draftCharades('deck-1', 'A', 'B', 'fixed-id');
...
describe('unlockCharades', () => {
  it('deals the given titles and flips the lock', () => {
    const drafted = draftCharades('deck-1', 'A', 'B');
...
describe('currentTeamIndex', () => {
  it('alternates strictly, team A first', () => {
    let state = unlockCharades(draftCharades('deck-1', 'A', 'B'), titles);
    const seen: number[] = [];
    for (let i = 0; i < 4; i++) {
...
describe('awardRound / skipRound', () => {
  it('awarding increments the given team score and advances the round', () => {
    const state = unlockCharades(draftCharades('deck-1', 'A', 'B'), titles);
...
describe('isCharadesComplete', () => {
  it('is false while locked, even with no titles', () => {
    expect(isCharadesComplete(draftCharades('deck-1', 'A', 'B'))).toBe(false);
  });
  ...
  it('handles a deck dealt with fewer than the usual title count', () => {
    let state = unlockCharades(draftCharades('deck-1', 'A', 'B'), titles.slice(0, 3));
    expect(isCharadesComplete(state)).toBe(false);
