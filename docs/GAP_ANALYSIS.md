...
| QR reveal | IMPLEMENTED | Live-verified (decoded a rendered QR image against the real dealt title, three rounds) | The shared screen never renders the round's title — only a QR code linking to `/charades/reveal`, which any phone's stock camera opens as a normal link. No camera permission inside the app itself. |
...
| Web | IMPLEMENTED | Live Playwright verification | `app/landing.tsx` is a dedicated bilingual, RTL-aware entry point served only on `Platform.OS === 'web'`; native installs still land on `/home`, which is Charades' own hub screen. |
