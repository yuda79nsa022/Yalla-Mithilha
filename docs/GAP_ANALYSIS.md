...
| --- | --- | --- | --- |
| Party Game | IMPLEMENTED | 162 app tests (`npm test`) | Fully offline, no account, engine is framework-free and config-table driven. |
| Mini-Games | IMPLEMENTED | Covered by the 162 above (`roundController`, `scoring`, `selector`) | Seven mini-games, one config row each; adding an eighth is a config row plus a renderer. |
| Offline Mode | IMPLEMENTED | `catalogueApi.test.ts` | Party Game needs no network at all. Board Game fetches a catalogue on startup and falls back to a bundled copy if that fails. One known bug below. |
| Save/Resume | IMPLEMENTED | `board.test.ts`, `boardGames.routes.test.ts` (server) | Board state carries a client-generated id; resuming after a restart replays the same id and `consumeCreditForBoardGame` no-ops on the repeat, so a resume never double-charges. |
| Board Game | IMPLEMENTED | `board.test.ts` + full server test suite | Draft → checkout → consume-credit → play → complete, end to end, server-authoritative at every money-relevant step. |
...
| Payments/KNET | PARTIAL | `boardGames.routes.test.ts`, `credits.test.ts` | `PaymentProvider` interface plus a fully working `MockPaymentProvider` — the checkout/confirm/fail flow is real and idempotent. Swapping in a real `KnetPaymentProvider` is explicitly deferred (product decision: no live merchant credentials yet); the interface is the seam for it. |
| Game Credits | IMPLEMENTED | `credits.test.ts` | Server-authoritative, append-only ledger (`SUM(grants) - SUM(consumes)`, never a mutable counter). Replaced an earlier client-side counter that could not be trusted. |
| Catalogue | IMPLEMENTED | `catalogue.routes.test.ts`, `db.test.ts` | Draft/Published/Archived states; a category needs both `published` status and 6/6 complete tiles to reach `GET /catalogue` — two independent gates. |
| Admin CMS | PARTIAL | `admin.routes.test.ts`, `adminUsers.routes.test.ts`, `adminPlayers.routes.test.ts` | Category/tile CRUD, image upload, staged import, audit log, all functionally complete. The admin UI itself (`server/public/index.html`) is English-only — the bilingual requirement was implemented across the player-facing app (`app/`, `src/i18n/`) but not extended to this internal tool. Flagged here rather than silently left out. |
| Imports | IMPLEMENTED | `import.routes.test.ts`, `parseTitles.test.ts` | Upload → preview → confirm → commit for docx/xlsx/pdf title lists, with duplicate detection surfaced for admin review before anything is written. Commit recomputes empty slots itself rather than trusting the preview, so a second admin editing concurrently can't cause a lost update. |
...
| Authorization Security | IMPLEMENTED | Every `*.routes.test.ts` file exercises this | `requireAdminSession`/`requirePlayerSession` gate every route that needs them; payment and board-game operations additionally check the row's `playerId` against the session before acting, tested explicitly (e.g. one player can't confirm or fail another player's payment). |
...
| Payment Security | IMPLEMENTED (within Mock scope) | `boardGames.routes.test.ts`, `credits.test.ts` | Re-read end to end this pass: `confirmPayment`/`failPayment` use a conditional `UPDATE ... WHERE status='initiated'` as a one-shot lock, so a webhook retry can't double-grant; `checkout`/`confirm`/`fail` all verify the payment's `playerId` matches the caller's session before acting; `product` is enum-validated against `PRODUCTS`, never taken as a free string. Real-provider signature verification is out of scope until a real provider exists (see Payments/KNET). |
| File/Import Security | IMPLEMENTED, one accepted low-priority gap | `categoryImage.routes.test.ts`, `import.routes.test.ts` | Both upload routes use `multer.memoryStorage()` with a size cap; image upload enforces a MIME-type allowlist via `fileFilter` and never trusts the client's filename — the stored name is `crypto.randomUUID()` plus an extension chosen from a server-side map, and `deleteUploadedFile()` runs every path through `path.basename()` before touching disk. docx/xlsx/pdf parsing extracts text (regex over raw XML for docx, `exceljs`/`pdf-parse` for the others) rather than resolving a full XML/entity graph, so there's no XXE path. Accepted gap: no explicit cap on decompressed zip size, so a crafted docx/xlsx could be a zip bomb; low priority because both import routes are admin-only (authenticated, trusted actor), not public attack surface. |
| Database Security | IMPLEMENTED | Whole server suite (168 tests) | Every query in `db.ts` is parameterized (`db.prepare(...).run({...})`/`.get({...})`) — no string-concatenated SQL anywhere in the codebase. |
...
| Privacy | IMPLEMENTED | — (documentation + `wipeEverything` covered by app tests) | Root `README.md`'s privacy section accurately lists the three things that talk to the backend (catalogue fetch, optional account creation, report sync) and states plainly that guest play needs none of them. One-tap local wipe exists. No camera/mic/contacts/location permission is declared. Analytics events are typed so a name or prompt string cannot be attached. |
...
the app running as a web page (`/catalogue`, `/players`, `/board-games`,
`/reports`) and left closed (no CORS headers, same-origin only) on
